...
UNAPPROVED_MSG = "**نظام حماية يـــمنثون الخاص بـ {ON}!**\n\n{UND}\n\nلديك {warn}/{twarn} تحذيرات"
if jmdB.get_key("PM_TEXT"):
    UNAPPROVED_MSG = (
        "**نظام حماية يـــمنثون الخاص بـ {ON}!**\n\n"
        ...
        if not keym.contains(user.id) and event.text != UND:
            ...
            if user.id in LASTMSG:
                ...
                if event.text != prevmsg:
                    if "نظام حماية يـــمنثون" in event.text or "**نظام حماية يـــمنثون" in event.text:
                        return
    ...
    async def approvepm(apprvpm):
        ...
        if user.id in DEVLIST:
            return await apprvpm.eor(
                "**⌔∮ هذا هو مطور سورس يـــمنثون هو بالأصل مسموح له بالدردشة  !**",
            )
...
async def in_pm_ans(event):
    ...
    try:
        ...
        msg_ = "**⌔∮ نظام حماية يـــمنثون الخاص بـ {OWNER_NAME}**"
    wrns = f"{warns}/{WARNS}"
    buttons = [
        [
        ...
        if res:
            res = [
                await event.builder.document(
                    res,
                    title="نظام حماية يـــمنثون الأنلاين",
